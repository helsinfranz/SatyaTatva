...
export default async function handler(req, res) {
  try {
    ...
    if (page === 1) {
      return res
        .status(200)
        .json({ images: encodedImages, totalPages: totalPages });
    }
...
async function renderPageToImage(page) {
  try {
    ...
    return removeWhiteBackground(imageData);
...
function removeWhiteBackground(imageData) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (r > 240 && g > 240 && b > 240) {
      ...
      // If the pixel is not transparent, add its index to the array
      nonTransparentPixels.push(i / 4); // Divide by 4 to get the pixel index
    }
  }
